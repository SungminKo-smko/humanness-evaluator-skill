...
def get_chain_oasis_peptides(chain, params: OASisParams):
    ...
    response = requests.post(params.oasis_db_path, json={
        "peptides": [peptide for pos, peptide in pos_peptides],
        "chain_type": oas_filter_chain
    })
    response.raise_for_status()
    api_data = response.json()
    
    num_total_oas_subjects = api_data['num_total_oas_subjects']
    oas_hits = pd.DataFrame(api_data['hits'])
    ...
    return {
        pos: parse_peptide_humanness(
            pep,
            oas_hits=oas_grouped.get_group(pep) if (oas_grouped is not None and pep in oas_grouped.groups) else pd.DataFrame(columns=['peptide', 'subject', 'count']),
